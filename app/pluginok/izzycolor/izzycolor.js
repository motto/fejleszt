...
if(typeof imageUrl === 'undefined')
	var imageUrl='app/pluginok/izzycolor/images/color.png';
...
function iColorShow(i,i2){
	...
	//var ttt=eIC.top+(jQuery("#"+i).outerHeight())+50;
	var offset = $('#'+i2).offset();
	var offset_top = offset.top-50;
	jQuery("#"+i).css({'z-index':'9999','position':'relative'});
	jQuery("#izzyColor").css({'top':offset_top+"px",'left':(eIC.left)+"px",'position':'absolute'}).fadeIn("fast");
...
this.izzyColor=function(){
	...
	jQuery("input.izzyColor").each(function(i){
		...
		.after('<a href="javascript:void(null)" id="icp_'+this.id+'" onclick="iColorShow(\''+this.id+'\',\'icp_'+this.id+'\')"><img id="img_'+this.id+'" src="'+imageUrl+'" style="border:0;margin:0 0 0 3px" align="absmiddle" ></a>')
